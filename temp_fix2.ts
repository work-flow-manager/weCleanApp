...
export async function POST(
  ...
  try {
    ...
    if (teamMember.profiles) {
      // Get the profile ID safely, handling both array and object cases
      let profileId = null;
      
      if (Array.isArray(teamMember.profiles)) {
        if (teamMember.profiles.length > 0 && teamMember.profiles[0]) {
          profileId = teamMember.profiles[0].id;
        }
      } else if (typeof teamMember.profiles === 'object' && teamMember.profiles !== null) {
        profileId = (teamMember.profiles as any).id;
      }
